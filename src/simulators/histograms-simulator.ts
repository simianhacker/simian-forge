...
export class HistogramsSimulator extends BaseSimulator<
  HistogramsConfig,
  HistogramsMetrics,
  HistogramsDocument
> {
  ...
  protected formatMetrics(
    metrics: HistogramsMetrics
  ): FormatterResult<HistogramsDocument>[] {
        ...
        try {
          ...
          for (const [name, template] of Object.entries(componentTemplates)) {
            try {
              ...
              console.warn(
                `Failed to create component template ${name}:`,
                error
              );
            }
          }
        ...
        template: {
          mappings: {
            properties: {
              "@timestamp": {
                type: "date",
                meta: { description: "Sample timestamp" },
              },
              "entity.id": {
                type: "keyword",
      ...
      "histograms-settings": {
        template: {
          settings: {
            index: {
              mode: "time_series",
              "time_series.start_time": new Date(
